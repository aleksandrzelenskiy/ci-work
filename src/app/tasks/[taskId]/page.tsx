...
export default function TaskDetailPage() {
  ...
  // ---- estimate link & visible attachments (estimate removed from attachments for everyone) ----
  const allAttachments = task.attachments ?? [];
  const estimateRegex = /(estimate|смета)/i;

  const estimateUrl =
      allAttachments.find((url) => {
        const name = (url.split('/').pop() ?? '').toLowerCase();
        return estimateRegex.test(name);
      }) ?? null;

  const otherAttachments = allAttachments.filter((url) => {
    const name = (url.split('/').pop() ?? '').toLowerCase();
    return !estimateRegex.test(name);
  });
          ...
          <Grid item xs={12} md={6}>
            <Box sx={{ mb: 3 }}>
              <Accordion defaultExpanded>
                ...
                <AccordionDetails>
                  {/* Ссылка на смету — только для ролей, отличных от executor */}
                  {userRole !== 'executor' && estimateUrl && (
                      <Box sx={{ mb: 1 }}>
                        <Button
                            component="a"
                            href={estimateUrl}
                  ...
                  {userRole === 'admin' && (
                      ...
                      >
                        ...
                        {task.orderUrl && task.orderNumber && task.orderDate && (
                            <Button
                                component="a"
                                href={task.orderUrl}
                                download={`${task.orderNumber} от ${dayjs(task.orderDate).format(
                                    'DD.MM.YYYY'
                                )}${
                                    (() => {
                                      const cleanUrl = task.orderUrl.split('?')[0].split('#')[0];
