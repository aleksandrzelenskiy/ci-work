...
// ширина картинки после ресайза (нужна для SVG)
            const { width = 900, height = 600 } = await img.metadata();
            const overlayHeight = Math.min(80, Math.round(height * 0.2)); // ≤20 % кадра, но не >80

            const overlaySvg = `
<svg width="${width}" height="${overlayHeight}">
  <rect width="100%" height="100%" fill="black" opacity="0.6"/>
  <text x="20" y="${overlayHeight * 0.4}"
        font-size="${overlayHeight * 0.35}"
        font-family="Arial, sans-serif"
        fill="white">
    ${date} | Task: ${task} | BS: ${baseId}
  </text>
  <text x="20" y="${overlayHeight * 0.8}"
        font-size="${overlayHeight * 0.35}"
        font-family="Arial, sans-serif"
        fill="white">
